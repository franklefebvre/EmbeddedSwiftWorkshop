...
final class BluetoothConnectionManager<Value: EndianCapable>: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    init(serviceUUID: CBUUID, characteristicUUID: CBUUID, valueType: Value.Type) {
        ...
        self.discoveryService.callback = discoveryServiceCallback
    }
...
extension BluetoothConnectionManager {
    func startScan() {
        state = .scanning
        readValue = nil
        discoveryService.showPicker(serviceUUID: serviceUUID)
    }
...
//        discoveryService.dismiss()
    ...
    func connect(identifier: UUID) {
        ...
        remotePeripheral = peripheral // must keep ref
...
extension BluetoothConnectionManager {
    func discoveryServiceCallback(_ event: DiscoveryService.Event) {
        switch event {
        case .activated(peripherals: let peripherals):
            state = .discovered(peripherals: peripherals)
        case .selected(let peripheral):
            connect(identifier: peripheral.uuid)
        }
    }
}
